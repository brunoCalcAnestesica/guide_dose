...
class BlockedDayDao {
  ...
  Future<void> insert(Map<String, dynamic> row) async {
    final db = await _db;
    await db.insert('blocked_days', row,
  ...
  Future<void> insertBatch(List<Map<String, dynamic>> rows) async {
    ...
    for (final row in rows) {
      batch.insert('blocked_days', row, conflictAlgorithm: ConflictAlgorithm.replace);
    }
  ...
  Future<Map<String, Map<String, dynamic>>> getByIdsMap(List<String> ids) async {
    ...
    for (var i = 0; i < ids.length; i += 500) {
      ...
      final rows = await db.query('blocked_days', where: 'id IN ($placeholders)', whereArgs: chunk);
      for (final row in rows) {
  ...
  Future<void> markSyncedBatch(List<String> ids) async {
    ...
    final batch = db.batch();
    for (final id in ids) {
      batch.update('blocked_days', {'sync_status': 'synced'}, where: 'id = ?', whereArgs: [id]);
    }
    await batch.commit(noResult: true);
  }
  ...
  Future<void> update(String id, Map<String, dynamic> row) async {
    final db = await _db;
    await db.update('blocked_days', row, where: 'id = ?', whereArgs: [id]);
  }

  Future<Map<String, dynamic>?> getById(String id) async {
    final db = await _db;
    final rows =
        await db.query('blocked_days', where: 'id = ?', whereArgs: [id]);
    return rows.isEmpty ? null : rows.first;
  }

  Future<void> delete(String id) async {
    final db = await _db;
    await db.delete('blocked_days', where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<List<Map<String, dynamic>>> getDeletedSync() async {
    final db = await _db;
    return db.query('blocked_days', where: "sync_status = 'deleted'");
  }
  ...
  Future<List<Map<String, dynamic>>> getPendingSync() async {
    final db = await _db;
    return db.query('blocked_days', where: "sync_status = 'pending'");
  }

  Future<void> markSynced(String id) async {
    final db = await _db;
    await db.update('blocked_days', {'sync_status': 'synced'},
